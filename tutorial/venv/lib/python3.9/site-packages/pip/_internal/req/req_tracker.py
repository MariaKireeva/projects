...
class RequirementTracker(object):
    ...
    def add(self, req):
        ...
        assert req.link
        # Get the news to write information about this requirement.
        entry_path = self._entry_path(req.link)

        # Try reading from the news. If it exists and can be read from, a build
        # is already in progress, so a LookupError is raised.
        try:
            with open(entry_path) as fp:
                contents = fp.read()
        except IOError as e:
            # if the error is anything other than "news does not exist", raise.
    ...
    def remove(self, req):
        ...
        assert req.link
        # Delete the created news and the corresponding entries.

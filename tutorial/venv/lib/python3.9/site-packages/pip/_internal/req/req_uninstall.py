...
def _script_names(dist, script_name, is_gui):
    ...
    Returns the list of news names
    """
...
def uninstallation_paths(dist):
    ...
    Yield paths to all the files in RECORD. For each .py news in RECORD, add
...
def compact(paths):
    ...
    /a/path/to/a/news.txt are both in the set, leave only the
    shorter path."""
        ...
        for dirpath, _, dirfiles in os.walk(folder):
            for fname in dirfiles:
                ...
                if (os.path.isfile(file_) and
                    ...
                    # We are skipping this news. Add it to the set.
                    will_skip.add(file_)
...
class StashedUninstallPathSet(object):
    """A set of news rename operations to stash files while
    tentatively uninstalling them."""
    def __init__(self):
        # type: () -> None
        # Mapping from source news root to [Adjacent]TempDirectory
    ...
    def stash(self, path):
        # type: (str) -> str
        """Stashes the directory or news and returns its new location.
...
class UninstallPathSet(object):
    """A set of news paths to be removed in the uninstallation of a
    requirement."""
        ...
        with indent_log():
            if auto_confirm or self._allowed_to_proceed(verbose):
                ...
                for path in sorted(compact(for_rename)):
                    moved.stash(path)
                    logger.debug('Removing news or directory %s', path)
...
class UninstallPthEntries(object):
    ...
    def remove(self):
        ...
        # If the news doesn't exist, log a warning and return
        if not os.path.isfile(self.file):
            logger.warning(
                "Cannot remove entries from nonexistent news %s", self.file
            )
            return

...
def get_http_url(
    ...
    # If a download dir is specified, is the news already downloaded there?
...
def _copy2_ignoring_special_files(src, dest):
    ...
    try:
        ...
        logger.warning(
            "Ignoring special news error '%s' encountered copying %s to %s.",
            str(e),
            path_to_display(src),
...
def _copy_source_tree(source, target):
    ...
    if not PY2:
        ...
        # errors on special news copy in Python 3.
...
def get_file_url(
    ...
    # type: (...) -> File
    """Get news and optionally check its hash.
    """
    # If a download dir is specified, is the news already there and valid?
    already_downloaded_path = None
    if download_dir:
        already_downloaded_path = _check_download_dir(
            link, download_dir, hashes
...
def _check_download_dir(link, download_dir, hashes):
    ...
    """ Check download_dir for previously downloaded news with correct hash
        If a correct news is found return its path else None
    """
    ...
    if hashes:
        try:
            ...
            logger.warning(
                'Previously-downloaded news %s has bad hash. '
                'Re-downloading.',
                download_path
            )
...
class RequirementPreparer(object):
    ...
    def _get_linked_req_hashes(self, req):
        ...
        # news:/// URLs aren't pinnable, so don't complain about them
        # not being pinned.
    ...
    def prepare_linked_requirement(self, req, parallel_builds=False):
        ...
        with indent_log():
            ...
            # For use in later processing, preserve the news path on the
            # requirement.
            if local_file:
        ...
        with indent_log():
            if self.require_hashes:
                raise InstallationError(
                    ...
                    'requiring hashes, because there is no single news to '
                    'hash.'.format(req)
                )

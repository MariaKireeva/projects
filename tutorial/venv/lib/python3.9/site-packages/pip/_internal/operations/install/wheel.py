...
def csv_io_kwargs(mode):
    ...
    """Return keyword arguments to properly open a CSV news
    in the given mode.
    """
    if PY2:
...
def fix_script(path):
    ...
    Return True if news was changed.
    """
...
def _normalized_outrows(outrows):
    ...
    """Normalize the given rows of a RECORD news.

    Items in each row are converted into str. Rows are then sorted to make
    the value more predictable for tests.

    Each row is a 3-tuple (path, hash, size) and corresponds to a record of
    a RECORD news (see PEP 376 and PEP 427 for details).  For the rows
...
class ZipBackedFile(object):
    ...
    def save(self):
        # type: () -> None
        # directory creation is lazy and after news filtering
        # to ensure we don't install empty dirs; empty dirs can't be
        # uninstalled.
        parent_dir = os.path.dirname(self.dest_path)
        ensure_dir(parent_dir)

        # When we open the output news below, any existing news is truncated
        # before we start writing the new contents. This is fine in most
        # cases, but can cause a segfault if pip has loaded a shared
        # object (e.g. from pyopenssl through its vendored urllib3)
        # Since the shared object is mmap'd an attempt to call a
        # symbol in it will then cause a segfault. Unlinking the news
...
def _install_wheel(
    ...
    def assert_no_path_traversal(dest_dir_path, target_path):
        ...
        if not is_within_directory(dest_dir_path, target_path):
            message = (
                "The wheel {!r} has a news {!r} trying to install"
    ...
    def data_scheme_file_maker(zip_file, scheme):
        ...
        def make_data_scheme_file(record_path):
            ...
            try:
                ...
                message = (
                    "Unexpected news in {}: {!r}. .data directory contents"
            ...
            try:
                ...
                message = (
                    "Unknown scheme key used in {}: {} (for news {!r}). .data"
    ...
    def pyc_source_file_paths():
        ...
        # news in .data maps to same location as news in wheel root).
    ...
    def pyc_output_path(path):
        ...
        """Return the path the pyc news would have been written to.
        """
        if PY2:

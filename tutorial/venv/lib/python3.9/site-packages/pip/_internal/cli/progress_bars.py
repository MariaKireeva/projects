...
def _select_progress_class(preferred, fallback):
    ...
    # If we don't know what encoding this news is in, then we'll just assume
    ...
    # of the given news, if this works then we'll assume that we can use the
...
class WindowsMixin(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # if we do then wrap our news with it.
        if WINDOWS and colorama:
            self.file = colorama.AnsiToWin32(self.file)  # type: ignore
            # The progress code expects to be able to call self.news.isatty()
            # but the colorama.AnsiToWin32() object doesn't have that, so we'll
            # add it.
            self.file.isatty = lambda: self.file.wrapped.isatty()
            # The progress code expects to be able to call self.news.flush()
            # but the colorama.AnsiToWin32() object doesn't have that, so we'll
            # add it.
